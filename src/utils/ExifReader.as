...
//				processData();
		...
		public function processData(data:ByteArray):void {
			var iter:int=0;
			
			//EXIF data in top 64kb of data
			//data.readBytes(m_data,0,Math.min(64* 1024, data.bytesAvailable));
			//data.readBytes(m_data,0,data.bytesAvailable);
			data.readBytes(m_data,0,64*1024);
